@php
    $isRegister = $purpose === \App\Services\OtpService::REGISTER;
    $heading = $isRegister ? 'Verify your new account' : 'Reset your password safely';
    $intro = $isRegister
        ? 'Use this one-time password to complete your City Go Remit account registration.'
        : 'Use this one-time password to reset your City Go Remit account password.';
@endphp
<!DOCTYPE html>
<html lang="en">
<head>
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background:#ffffff;border:1px solid #dfe5ec;border-radius:20px;overflow:hidden;">
                    ...
                    <tr>
                        <td style="padding:28px;">
                            <p style="margin:0 0 12px;font-size:13px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:#60707d;">Your OTP Code</p>
                            <div style="background:#f8fafc;border:1px solid #dfe7ef;border-radius:18px;padding:22px;text-align:center;">
                                <div style="font-size:38px;line-height:1;font-weight:800;letter-spacing:8px;color:#00503a;">{{ $code }}</div>
                            </div>
